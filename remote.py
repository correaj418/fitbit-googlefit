...
class Remote:
    ...
    def SyncFitbitLogToGoogleFit(self, dataType, date_stamp):
        ...
        fitbitLogs = self.ReadFromFitbit(callMethod, base_date=date_stamp, end_date=date_stamp)[
            resp_id]
    ...
    def SyncFitbitSleepToGoogleFit(self, date_stamp):
        ...
        for fit_sleep in fitbitSleeps:
            ...
            next_date_stamp = (datetime.strptime(date_stamp, DATE_FORMAT) + timedelta(1)).strftime(
                DATE_FORMAT)
            ...
            google_session = self.convertor.ConvertGFitSleepSession(googlePoints,
                                                                    fit_sleep['logId'])
    ...
    def SyncFitbitActivitiesToGoogleFit(self, start_date='', callurl=None):
        ...
        if len(startTimeMillis) > 0:
            print("Synced {} exercises between : {} -- {}".format(len(activities),
                                                                  datetime.fromtimestamp(min(
                                                                      startTimeMillis) / 1000).strftime(
                                                                      '%Y-%m-%d'),
                                                                  datetime.fromtimestamp(max(
                                                                      endTimeMillis) / 1000).strftime(
                                                                      '%Y-%m-%d')))
        else:
